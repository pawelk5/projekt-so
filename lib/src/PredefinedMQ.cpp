...
RestaurantMQ GetRestaurantMQ(pid_t restaurantPID, bool worker, const std::function<bool()>& errorHandler, bool blocking) {
    RestaurantMQ t_mq = std::make_shared<MessageQueue<RestaurantMQMessage>>();
    MessageQueueParams t_params;
    t_params.blocking = true;
    t_params.create = worker;
    t_params.msqName = std::to_string(restaurantPID) + "-restaurant";
    t_params.maxMsgCount = DEFAULT_MAX_MSQ_SIZE;
